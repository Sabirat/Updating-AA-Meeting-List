		...
		if anurl=="$$":
			...
			for link in links:																#######href in a######################
				...
				try:
					...
					if IsValidWebpage(page,linkhome,urls):
			...
			submit_BUTTON_XPATH = '//*[@type="submit"|@type="Submit"]'
			buttons = browser.find_elements_by_xpath(submit_BUTTON_XPATH)
			print "submit length:"+str(len(buttons))			
			...
			submit_BUTTON_XPATH = '//*[@type="button"|@type="Button"]'
			buttons = browser.find_elements_by_xpath(submit_BUTTON_XPATH)
			...
			print "form length:"+str(len(links))
...
document = ElementTree.parse( 'tempxml.xml' )
